...
static CString GetStringSecNew(LPCTSTR s, int n1, BOOL Warn, TCHAR ch)
{
	...
	for (int i = 0; i < n; ++i) {
		...
		if (p == nullptr) {
			if (Warn)
				RAISE_AMN_EXCEPTION(TString(_T("String section not found: [")) + iToStr(n) + _T("]: ") +
									(s != nullptr ? s : _T("nullptr")));
			pStr = p;
			oldN = n1;
			return "";
		}
...
CString AMNCORE_API Amn::Sys::Text::Conversion::ToString(const Date& d)
{
	...
	switch (SysFormat.eDateFormat) {
		case Format::DF_LEADINGZERO:
			os << setfill(L'0') << setw(2) << d.Day() << dateSep << setfill(L'0') << setw(2) << d.Month() << dateSep
		...
		case Format::DF_LEADINGZEROYY:
			os << setfill(L'0') << setw(4) << d.Year() << dateSep << setfill(L'0') << setw(2) << d.Month() << dateSep
...
CString AMNCORE_API Amn::Sys::Text::Conversion::ToString(const Time& t)
{
	...
	switch (SysFormat.eDateFormat) {
		case Format::DF_LEADINGZEROYY:
		case Format::DF_LEADINGZERO:
			os << setfill(L'0') << setw(2) << t.Hour() << timeSep << setfill(L'0') << setw(2) << t.Minute() << timeSep
...
CString AMNCORE_API Amn::Sys::Text::Conversion::SQLdToStr(const Date& dt)
{
	LPCTSTR		   dtSep = L"-";
	wostringstream os;
	os << "'" << setfill(L'0') << setw(4) << dt.Year() << dtSep << setfill(L'0') << setw(2) << dt.Month() << dtSep
